...
//        }
    ...
    private Response handleAuthorizationException(AuthorizationException authorizationException) {
        return Response.status(Response.Status.UNAUTHORIZED).entity(authorizationException).type(ContentType.APPLICATION_JSON_UTF_8).build();
    }

    protected Response handleApiException(ApiException apiException) {
        return Response.status(Response.Status.OK).entity(apiException).type(ContentType.APPLICATION_JSON_UTF_8).build();
    }

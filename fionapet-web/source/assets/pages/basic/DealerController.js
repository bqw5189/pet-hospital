...
// 经销商与生产商
angular.module('fiona').controller('DealerController', function($scope, $http, commons) {
    ...
    $scope.selectAll= function () {
        angular.forEach($scope.dealers, function (dealer) {
            $scope.selection[dealer.id] = $scope.selectedall;
        })
    ...
    $scope.search = function () {
        ...
        $http.post('/business/api/v2/dealers/page', {'pageSize':$scope.pagination.pageSize, 'pageNumber':$scope.pagination.pageNumber, 'filters':$scope.filters}).success( function ( data, status, headers, config ) {
            console.log( data );

            $scope.dealers = data.data.content;
    ...
    $scope.update = function (id) {
        ...
        {
            angular.forEach($scope.dealers, function(data,index,array){
                if(data.id== id)
                {
                    $scope.dealer = data;
                }
    ...
    $scope.dealersubmit = function () {
        ...
        $scope.dealerform.submitted = true;

        if ($scope.dealerform.$valid) {

            console.log('submit: ' + $scope.dealer.id);

            $http.post('/business/api/v2/dealers', $scope.dealer).success( function ( data, status, headers, config ) {
                console.log( data );
            }).error(function(data,status,headers,config) { //     错误
                alert("Dealersubmit Error");
            });
        } else {

            alert($scope.dealerform.$error);
    ...
    $scope.remove= function (id) {
        ...
        $http.delete('/business/api/v2/dealers/' + id).success(function(data, index, array){
            commons.danger("删除成功");
        }).error(function (data) {
            commons.danger("删除失败");
        });
